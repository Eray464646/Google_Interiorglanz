      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <FadeIn delay={0.2}>
          <div className="w-full max-w-5xl mx-auto mb-12">
             <div className="relative bg-brand-950 text-white rounded-2xl md:rounded-3xl p-8 md:p-12 shadow-[0_20px_50px_rgba(0,0,0,0.3)] border border-brand-900 overflow-hidden transform hover:-translate-y-1 transition-transform duration-300 group">
                <div className="absolute top-0 right-0 w-[400px] h-[400px] bg-brand-gold/10 rounded-full blur-[100px] -mr-20 -mt-20"></div>
                
                {/* Bestseller Badge - Moved higher (top-2) and added padding to title below to prevent overlap */}
                <div className="absolute top-2 right-2 md:top-6 md:right-12 bg-brand-gold text-brand-950 text-[10px] md:text-xs font-extrabold px-3 py-1 md:px-4 md:py-1.5 rounded-full uppercase tracking-widest shadow-lg flex items-center gap-1 animate-pulse z-20">
                   <Star size={10} className="md:w-3 md:h-3" fill="black" /> Bestseller
                </div>

                <div className="relative z-10 flex flex-col lg:flex-row gap-10 lg:items-center">
                   <div className="lg:w-1/2 pt-2 md:pt-0">
                      {/* Added pr-24 on mobile to force text wrap before hitting the badge */}
                      <h4 className="text-3xl md:text-4xl font-bold mb-4 text-white group-hover:text-brand-gold transition-colors pr-24 md:pr-0">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-6xl mx-auto mb-24">
          {interiorPackages.map((pkg, index) => (
            <FadeIn key={index} delay={index * 0.15}>
              <div 
                className="flex flex-col bg-white text-brand-950 rounded-2xl p-8 shadow-lg border border-gray-100 hover:border-brand-gold/50 hover:shadow-xl transition-all duration-300 group h-full"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
             {ADDONS.map((addon, index) => (
                <FadeIn key={index} delay={index * 0.2} direction={index === 0 ? "right" : "left"}>
                  <div className="relative bg-brand-900 text-white rounded-2xl p-8 overflow-hidden hover:transform hover:-translate-y-1 transition-all duration-300 shadow-xl border border-brand-800 group">
